...
class Data(object):
    ...
    def __init__(self, data_folder, modalities_to_load=None, dataset='ISLES', trim_and_downsample=False):
        ...
        elif self.dataset == 'BRATS15':
            self.num_vols = 54
            self.splits_file = 'data/BRATS15/splits.txt'
        ...
        if modalities_to_load is not None:
            ...
            self.modalities_to_load = ['T1', 'T2', 'Flair', 'T1c', 'Mask']
        elif dataset == 'BRATS15' or dataset == 'BRATS18':
            self.modalities_to_load = ['T1', 'T2', 'Flair', 'T1c', 'Mask']
        elif dataset == 'IXI':
        ...
        self.refDict = {'T1': self.T1, 'T2': self.T2, 'Flair': self.Flair, 'T1c': self.T1c, 'Mask': self.Mask}
    ...
    def load(self):
        ...
        self.Flair = self.refDict['Flair']
        self.T1c = self.refDict['T1c']
    ...
    def load_modality(self, modality, normalize_volumes=True, downsample=2, rotate_mult=0.0, shift_mult=0.0):
        ...
        elif self.dataset == 'BRATS15' or self.dataset == 'BRATS18':
            file_name = 'data/' + self.dataset + '/' + modality + '.npz'
            data = np.load(file_name)['arr_0']
        elif self.dataset == 'IXI':
...
if __name__ == '__main__':
    data = Data('data/BRATS15',  dataset='BRATS15', trim_and_downsample=True)
    data.load()
    data.select_for_ids('T2', [0])